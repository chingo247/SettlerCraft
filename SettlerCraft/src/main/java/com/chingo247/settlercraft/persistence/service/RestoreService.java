...
public class RestoreService {
    ...
    private void setRemovedAfter(World world, Timestamp timestamp) {
        ...
        try {
            ...
            if (!structures.isEmpty()) {
                Bukkit.getConsoleSender().sendMessage(SettlerCraft.MSG_PREFIX + "World " + world + " contains " + structures.size() + " that have an invalid status");
                ...
                while (it.hasNext()) {
                    ...
                    if (!manager.hasRegion(structure.getStructureRegion())) {
                        Bukkit.getConsoleSender().sendMessage(SettlerCraft.MSG_PREFIX + "Structure #" + structure.getId() + " was removed after last save");
                        reclaim(structure);
                        Bukkit.getConsoleSender().sendMessage(SettlerCraft.MSG_PREFIX + "Reclaimed region: " + structure.getStructureRegion());
                    }
    ...
    private void removeCreatedBefore(String world, Timestamp timestamp) {
        ...
        try {
            ...
            if (!structures.isEmpty()) {
                Bukkit.getConsoleSender().sendMessage(SettlerCraft.MSG_PREFIX + "World '" + world + "' has " + structures.size() + " structures that were placed after the last world save");
            }
            Iterator<Structure> it = structures.iterator();
            ...
            while (it.hasNext()) {
                ...
                Bukkit.getConsoleSender().sendMessage(SettlerCraft.MSG_PREFIX + "Region: " + structure.getStructureRegion() + " has been removed");
    ...
    private static boolean firstTime(World world) throws IOException {
        File worldsFolder = new File(SettlerCraft.getInstance().getDataFolder(), "Structures/restoreservice");
        worldsFolder.mkdirs();
