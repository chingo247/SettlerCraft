 ...
 */
package com.chingo247.settlercraft.main.structure.construction;

import com.chingo247.settlercraft.main.structure.construction.worldedit.StructureBlock;
import java.util.Comparator;

/**
 * A Pattern mode specifies the pattern in which the Structure will be constructed.
 * A Pattern makes it possible for the BlockPlacer to travel trough the schematic in smaller cubes,
 * creating a realistic effect. The thought behind this is that lots of block-changes on a smaller area (square) will have increased performance.
 * 
 * 
 * @author Chingo
 */
public class Pattern {
            ...
            public int compare(StructureBlock o1, StructureBlock o2) {
                ...
                if (v4 == 0) {
                    ...
                    if (v1 == 0) {
                        ...
                        if (v2 == 0) {
                            return getPiece(o1.getPosition().getBlockX(), square).compareTo(getPiece(o2.getPosition().getBlockX(), square));
                        }
                        return v2;
    ...
    /**
     * Creates a Pattern, which defines in which pattern the blocks are placed or removed
     * The layer height is equal to the height of the schematic
    ...
    public static Pattern createPattern(final int square) {
        return new Pattern(new Comparator<StructureBlock>() {
            ...
            public int compare(StructureBlock o1, StructureBlock o2) {
                ...
                if (v3 == 0) {
                    Integer v1 = getPiece(o1.getPosition().getBlockX(), square).compareTo(getPiece(o2.getPosition().getBlockX(), square));
                    if (v1 == 0) {
                        Integer v2 = getPiece(o1.getPosition().getBlockZ(), square).compareTo(getPiece(o2.getPosition().getBlockZ(), square));
                        if (v2 == 0) {
    ...
    /**
     * Creates a Pattern, which defines in which pattern the blocks are placed or removed.
     * There is no visible layering, blocks are placed/removed in squares of 16 (The ChunkSize)
     * @return The Pattern
     */
    ...
    /**
     * Creates a Pattern, which defines in which pattern the blocks are placed or removed.
     * There is visible layering, blocks are placed/removed in cubes of 32x32x32 (width x height x length) blocks
     * @return The Pattern
     */
    ...
    /**
     * Creates a Pattern, which defines in which pattern the blocks are placed or removed.
     * There is visible layering, blocks are placed/removed in cubes of 32x16x32 (width x height x length) blocks.
     * Just because this method is called fancy doesn't mean it has decreased performance. There is just more visible layering
     * @return The Pattern
     */
    ...
    public enum Mode {
        ...
        public static Mode getMode(int mode) {
            switch(mode) {
                ...
                case 2: return FANCY;
                default: throw new AssertionError("Unkown mode: " + mode);
            }
        }
