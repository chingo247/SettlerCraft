 ...
 */
package com.chingo247.structureapi.main.persistence.service;
...
public class StructureService extends AbstractService {
    ...
    public Structure getStructure(Location location) {
        ...
        Structure structure = query.from(qStructure)
                .where(qStructure.location().world.eq(location.getWorld().getName())
                        .and(qStructure.dimension().minX.loe(location.getPosition().getBlockX()))
                        .and(qStructure.dimension().maxX.goe(location.getPosition().getBlockX()))
                        .and(qStructure.dimension().minZ.loe(location.getPosition().getBlockZ()))
                        .and(qStructure.dimension().maxZ.goe(location.getPosition().getBlockZ()))
                        .and(qStructure.dimension().minY.loe(location.getPosition().getBlockY()))
                        .and(qStructure.dimension().maxY.goe(location.getPosition().getBlockY()))
    ...
    public Structure getOverlappingStructure(Structure structure) {
        ...
        Structure result = query.from(qStructure)
                .where(qStructure.location().world.eq(structure.getName())
    ...
    public List<Structure> getStructuresWithin(World world, Dimension dimension) {
        ...
        List<Structure> result = query.from(qStructure)
                .where(qStructure.location().world.eq(world.getName())
    ...
    public boolean hasStructuresWithin(String world, Dimension dimension) {
        QStructure qStructure = QStructure.structure;
        Session session = HibernateUtil.getSession();
        JPQLQuery query = new HibernateQuery(session);
