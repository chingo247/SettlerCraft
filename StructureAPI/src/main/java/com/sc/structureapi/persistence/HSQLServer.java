...
public class HSQLServer {
    ...
    public boolean isRunning() {
        ...
        try(Socket socket = new Socket(HOST, PORT)) {
            Bukkit.getConsoleSender().sendMessage(StructureAPIModule.MSG_PREFIX + "HSQL Server already running");        
            return true;
        } catch(IOException | HsqlException ex) {
            Bukkit.getConsoleSender().sendMessage(StructureAPIModule.MSG_PREFIX + "HSQL Server not running, startng it on port " + PORT);        
            return false;
        }
    }
    ...
    public void stop() {
        Bukkit.getConsoleSender().sendMessage(StructureAPIModule.MSG_PREFIX + "Stopping HSQL server");
        server.stop();
    }
